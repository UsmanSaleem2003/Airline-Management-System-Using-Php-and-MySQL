...
<div class="maindiv">
  ...
  <div class="text">
    <p class="t1">INTERNATIONAL TICKETING DETAILS</p>
  </div>
  <hr class="line2">
...
if($_SERVER['REQUEST_METHOD'] == 'POST'){
  ...
  $departingfromcountry = $_POST['departingfromcountry'];
...
{
  // if(empty($fname) || empty($mname) || empty($lname) || empty($email) || empty($areacode) || empty($phoneNumber) || empty($address) || empty($city) || empty($state) || empty($zipcode) || empty($country)|| empty($departuredate) || empty($departuretime) || empty($returndate) || empty($returntime) || empty($departingfromcountry) || empty($departingfromcity) || empty($destinationcountry) || empty($destinationcity) || empty($paymentmethod) || empty($cardnumber)){
  //   echo '
  //   <script>
  ...
  // else
  {
    $sql1 = "INSERT INTO `international_personal_info` (`FirstName`, `MiddleName`, `LastName`, `Email`, `Area_code`, `PhoneNumber`, `Address`, `City`, `State_Province`, `Zip_Postal_Code`, `Country`) Values ('$fname','$mname','$lname','$email','$areacode','$phoneNumber' , '$address', '$city', '$state', '$zipcode', '$country')";
    $result1 = mysqli_query($conn, $sql1);
    $sql2 = "INSERT INTO `international_ticket_info` (`Email`, `Departure_Date`, `Departure_Time`, `Return_Date`, `Return_Time`, `Departing_From_Country`, `Departing_From_City`, `Destination_Country`, `Destination_City`, `Airline`, `Card_Number`, `Payment_Method`) Values ('$email','$departuredate','$departuretime','$returndate','$returntime','$departingfromcountry' , '$departingfromcity', '$destinationcountry', '$destinationcity', '$airline', '$cardnumber', '$paymentmethod')";
...
else
{
  ...
  <form action="/Airline/Ticketing/next.php" method="post">
...
<div class="phone_details">
  <input type="text" class="city" name="zipcode">
...
<div class="depart1">
  ...
  <select id="list2" name="departingfromcountry">
        <option class="u" value="none">--Please Select</option>
        <option class="u" value="Pakistan">Pakistan</option>
        <option class="u" value="India">India</option>
        <option class="u" value="Iran">Iran</option>
      ...
      <input type="text" name="departingfromcity" placeholder="Enter City Name" class="city2">
...
<div class="depart1">
  ...
  <select name="destinationcountry" id="list2">
        <option class="u" value="none">--Please Select</option>
        <option class="u" value="Pakistan">Pakistan</option>
        <option class="u" value="India">India</option>
        <option class="u" value="Iran">Iran</option>
      ...
      <input type="text" name="destinationcity" placeholder="Enter City Name" class="city2">
      <p class="city2t">City</p>
      <p class="countryselecttext" >Country</p>
</div>
...
<input type="text" class="card_text" name="cardnumber" placeholder="Enter Card Number">
      ...
      <button type="submit" class="login-button2" class="login_link" name="luggage" onclick="Luggage()">Luggage Selection</button>
      <button type="submit" class="PricePlan" class="login_link" name="priceplan" onclick="PricePlan()">Price Plan</button>
</div>

  <hr class="line">
  <button type="submit" class="login-button" name="reserveflight" class="login_link" onclick="ReserveFlight()">Reserve Flight</button>
  <button type="button" class="back-button" name="back" class="login_link" onclick="Previous()">Back</button>
